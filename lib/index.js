...
var InsightAPI = function(options) {
  ...
  this.coinTicker = options.coinTicker || 'https://api.coinmarketcap.com/v1/ticker/aidpcoin/?convert=USD';
...
InsightAPI.dependencies = ['aidpd', 'web'];
...
InsightAPI.prototype.start = function(callback) {
  ...
  this.node.services.aidpd.on('tx', this.transactionEventHandler.bind(this));
  this.node.services.aidpd.on('block', this.blockEventHandler.bind(this));
...
InsightAPI.prototype.getPublishEvents = function() {
  return [
    ...
    },
    {
        ...
        scope: this,
        subscribe: function (emitter) {
            this.subscribe(emitter, 'aidp');
        }.bind(this),
        unsubscribe: function (emitter) {
            this.unsubscribe(emitter, 'aidp');
        },
        extraEvents: ['aidp/tx', 'aidp/block']
    }
  ];
};
...
InsightAPI.prototype.blockEventHandler = function(hashBuffer) {
  ...
  if (self.subscriptions.aidp.length) {
      ...
      this.blocksController.getBlockByHash(hashBuffer.toString('hex'), function (err, block) {
          ...
          async.mapSeries(block.tx, function(txid, next) {
              ...
              // Notify aidp subscribers
              for (var i = 0; i < self.subscriptions.aidp.length; i++) {
                  self.subscriptions.aidp[i].emit('aidp/block', {
                      block: block,
...
InsightAPI.prototype.transactionEventHandler = function(txBuffer) {
   ...
   if (this.subscriptions.inv.length || this.subscriptions.aidp.length) {
       ...
       if (this.subscriptions.aidp.length) {
           var transformedTrx = this.txController.transformRavenTransaction(tx);

           for (var i = 0; i < this.subscriptions.aidp.length; i++) {
               this.subscriptions.aidp[i].emit('aidp/tx', transformedTrx);
           }
